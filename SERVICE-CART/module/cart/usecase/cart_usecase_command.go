...
func (u *cartUseCase) CreateCart(ctx context.Context, request *pb.CreateCartRequest) (response *pb.CreateCartResponse, err error) {
	...
	if err != nil {
		if err = u.cartPublisher.PublishSagaMessage(ctx, utils.CreateCartEventSaga, sagaFailed); err != nil {
			return
		}
		return
	}
	...
	if err = u.cartMongoDBRepository.StartTransaction(ctx, session); err != nil {
		if err = u.cartPublisher.PublishSagaMessage(ctx, utils.CreateCartEventSaga, sagaSuccess); err != nil {
			return
		}
		return
	}
	...
	if err = u.cartMongoDBRepository.CommitTransaction(ctx, session); err != nil {
		err = u.cartPublisher.PublishSagaMessage(ctx, utils.CreateCartEventSaga, sagaFailed)
	}
	return
}
